...
import { Camera, Plus, Trash2, Check, Edit3, X, Heart } from 'lucide-react'
...
export default function Memories() {
  ...
  const [newMemory, setNewMemory] = useState({ title: '', date: '', description: '', mood: 'Heart' })
  ...
  const handleSubmit = (e) => {
    ...
    if (newMemory.title.trim() && newMemory.date) {
      ...
      setNewMemory({ title: '', date: '', description: '', mood: 'Heart' })
      setShowForm(false)
    }
  }

  const handleEdit = (memory) => {
    setNewMemory({ title: memory.title, date: memory.date, description: memory.description || '', mood: memory.mood })
