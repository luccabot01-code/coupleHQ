...
export default function Settings() {
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <div className={`rounded-2xl p-6 shadow-sm border mb-6 ${isDark ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-100'}`}>
        <h2 className={`font-semibold mb-4 flex items-center gap-2 ${isDark ? 'text-white' : 'text-gray-800'}`}>
          <Download className="w-5 h-5 text-green-500" /> {t('dataManagement')}
        </h2>
        <div className="space-y-4">
          <div className={`flex items-center justify-between p-4 rounded-xl ${isDark ? 'bg-gray-700' : 'bg-gray-50'}`}>
            <div>
              <p className={`font-medium ${isDark ? 'text-gray-200' : 'text-gray-700'}`}>{t('exportData')}</p>
              <p className={`text-sm ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>{t('downloadAllData')}</p>
            </div>
            <button
          ...
          <div className={`flex items-center justify-between p-4 rounded-xl ${isDark ? 'bg-gray-700' : 'bg-gray-50'}`}>
            <div>
              <p className={`font-medium ${isDark ? 'text-gray-200' : 'text-gray-700'}`}>{t('importData')}</p>
              <p className={`text-sm ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>{t('restoreFromBackup')}</p>
              {importStatus === 'success' && (
                <p className="text-sm text-green-500 mt-1">{t('importedSuccess')}</p>
              )}
              {importStatus === 'error' && (
                <p className="text-sm text-red-500 mt-1">{t('importFailed')}</p>
              )}
            </div>
            <div>
              <input
